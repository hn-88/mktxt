...
$is_buffering_off = ob_end_flush();

if ($is_buffering_off) {
  echo "Output buffering is turned off.<br>";
}
...
$fileurl = "http://192.168.127.10/mkcsv/TeluguStream".$playlistDatenodash.".csv"; // Replace with the name of your CSV file
$file = "../mkcsv/TeluguStream".$playlistDatenodash.".csv"; //
$filecontents = file_get_contents($fileurl);
file_put_contents($file, $filecontents);

$expfilename = "data/5-".$playlistDate.".txt";
...
function doImport($expfilename,$file) {
  ...
  $previousdescr = str_replace("'", '"', $data[0][2]);
  // we need to replace single quotes with double quotes since json does not like single quotes.
  //echo "<br>". $previousdescr . "<br>";
  $prevbflag = $data[0][5];
  $prevdflag = $data[0][6];
  ...
  for ($i=1; $i<count($data);$i++) {
    ...
    //ob_flush();
    //flush();
    $escdescr = str_replace("'", '"', $data[$i][2]);
    if ($escdescr==$previousdescr){
        //if ($data[i][5]=='1') {
          // bflag
          //if ($prevfids!='') {
          //  prevfids=prevfids+',';
        ...
        $previousdescr = str_replace("'", '"', $data[$i][2]);
        // we need to replace single quotes with double quotes since json does not like single quotes.
        $prevfids = '';
        $prevdfids='';
        $prevbflag=$data[$i][5];
    ...
    if ($notFoundString!="The filenames not found are \n\n") {
      ...
      mail('ganesh@sssmediacentre.org','files not found in SSSMC for '.$expfilename,$notFoundString);
    }
